...
import { ResponseRoll } from "../entities/responseRoll";
...
export class RpgUtils extends Rolling {
    ...
    */
    private checkDownUp(down:boolean, roll: number, check: number | undefined): boolean {
    ...
    */
    private isModified(roll: number, modified: number | undefined): number {
    ...
    */
    private reduceDices(down: boolean, success: number, keyDiceProperty: string | null = null): number {
        return this.dices.filter(dice => {
            return this.checkDownUp(down,  this.isModified(dice.roll, dice.rollModified), !keyDiceProperty ? success : dice[keyDiceProperty])
        }).length
    }
    ...
    */
    public headOrTails(times:number = 1):ResponseRoll {
        ...
        const countTails = arrRoll.filter(v => v == Constants.TAILS).length;
        const countHead = arrRoll.filter(v => v == Constants.HEAD).length;
        const success = countTails == countHead ? Constants.TIE : (countTails > countHead ? Constants.TAILS : Constants.HEAD);
        const rolls: ResponseRoll = {
            rolls: arrRoll,
            success: success,
            countSuccess: success == Constants.TAILS ? countTails : countHead,
    ...
    */
    public checkSuccess(success:number, dicesSuccess: number = 1, down:boolean = false):ResponseRoll | Error {
        if(this.dices.length == 0) return new Error(Constants.ERROR_NO_DICES);
        const response: ResponseRoll = {
            rolls: this.mapRolls(Constants.ROL),
            modifiedRolls: this.mapRolls(Constants.MODIFIED_ROL),
            success: this.reduceDices(down, success) >= dicesSuccess ? Constants.YES : Constants.NO,
            countSuccess: this.reduceDices(down, success),
            countFail: this.dices.length - this.reduceDices(down, success),
            countCritical: this.reduceDices(down, success, Constants.CRITICAL),
            countFailure: this.reduceDices(!down, success, Constants.FAILURE)
        }
        return response;
    }
} 